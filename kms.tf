...
resource "aws_kms_key" "superset_eastern_state_prod" {
  deletion_window_in_days = 7
}

resource "aws_kms_alias" "superset_eastern_state_prod_alias" {
  name = "alias/superset-eastern-state-prod"
  target_key_id = "${aws_kms_key.superset_eastern_state_prod.key_id}"
}
